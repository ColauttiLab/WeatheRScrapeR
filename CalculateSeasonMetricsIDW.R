...
files <- list.files("C:/Users/YIHANWU/Documents/2016 Queens/WeatherScraper/WeatherScraper/WeatherRawData", pattern = "NOAACommonGarden*")
files <- files[1:3]
...
for(year in (2008:2010)){ 
  ...
  for(Pop in PopData$Pop_Code[PopData$Year==year]){ # Cycle through pop_codes sampled in same year as GD data
    ...
    if(nrow(GeoDat)>=3){ # Only interpolate if at least 10 stations available
      # Interpolate GDD for Pop
      ### ALSO CHANGE THIS TO INTERPOLATE FOR GDD, GD, GDDs, GDs
      PopData$GD[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$GD)
      PopData$GDs[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$GDs)
      PopData$GDD[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$GDD)
      PopData$GDDs[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$GDDs)
      PopData$meanGDeg[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$meanGDeg)
      PopData$varGDeg[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$varGDeg)
      PopData$skewGDeg[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$skewGDeg)
      PopData$kurtGDeg[PopData$Pop_Code==Pop]<-idw(GeoDat$Dist, GeoDat$kurtGDeg)
