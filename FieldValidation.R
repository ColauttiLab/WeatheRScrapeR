...
for(year in (2003:2005)){ 
  ...
  for(Pop in FieldData$Pop_Code[FieldData$Year==year]){ # Cycle through pop_codes sampled in same year as GD data
    Cntr<-Cntr+1
    ...
    for (Stn in LocStns) { 
      ###
      yday<-as.numeric(FieldData$yday[FieldData$Pop_Code==Pop])
    ...
    if(nrow(GeoDat)>=5){ # Only interpolate if at least 10 stations available
      ...
      FieldData$GD[FieldData$Pop_Code==Pop]<-predict(tpsGD,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$GDs[FieldData$Pop_Code==Pop]<-predict(tpsGDs,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$GDD[FieldData$Pop_Code==Pop]<-predict(tpsGDD,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")      
      FieldData$GDDs[FieldData$Pop_Code==Pop]<-predict(tpsGDDs,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")   
      FieldData$meanGDeg[FieldData$Pop_Code==Pop]<-predict(tpsmeanGDeg,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$varGDeg[FieldData$Pop_Code==Pop]<-predict(tpsvarGDeg,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$skewGDeg[FieldData$Pop_Code==Pop]<-predict(tpsskewGDeg,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$kurtGDeg[FieldData$Pop_Code==Pop]<-predict(tpskurtGDeg,x=FieldData[FieldData$Pop_Code==Pop,c("Longitude","Latitude")],scale.type="unscaled")
      FieldData$numStns[FieldData$Pop_Code==Pop]<-nrow(GeoDat)
      # SAVE output
      write.csv(FieldData,"FieldData_wGDDtest.csv",row.names=F)
    }
    cat("***************\nIteration ",Cntr," of",length(unique(FieldData$Pop_Code)),"\nYear: ",year,"\nPop: ",Pop,"\n",Sys.time(),"seconds","\nGDD: ",FieldData$GDD[FieldData$Pop_Code==Pop],"\nGDays: ",FieldData$GDays[FieldData$Pop_Code==Pop],"\n***************")
